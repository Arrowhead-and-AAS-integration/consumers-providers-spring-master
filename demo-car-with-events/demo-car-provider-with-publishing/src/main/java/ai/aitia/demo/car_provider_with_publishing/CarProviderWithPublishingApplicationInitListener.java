...
public class CarProviderWithPublishingApplicationInitListener extends ApplicationInitListener {
	...
	private void publishDestroyedEvent() {
		...
		// Parse the JSON response to get the value of the submodel with idShort "temperature"
		try {
			ObjectMapper objectMapper = new ObjectMapper();
			JsonNode root = objectMapper.readTree(response.getBody());
			if (root.isArray() && root.size() > 0) {
				for (JsonNode submodel : root) {
					if ("temperature".equalsIgnoreCase(submodel.path("idShort").asText())) {
						temperatureValue = submodel.path("value").asText();
						logger.info("Submodel temperature value: " + temperatureValue);
						break;
					}
				}
			} else {
		...
		final String payload = String.valueOf(temperatureValue);
